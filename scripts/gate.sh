...
export TOOLCHAIN="${OXML_CLI_TOOLCHAIN:-1.98.0}"
MSRV="${OXML_CLI_MSRV:-1.86.0}"
...
# The examples assert their own output. They were not run by CI until
# 0.0.7, and two assertions in `inspect.sh` had been failing against
# the published `oxml` for a release by the time anyone looked.
