...
for dep in "${DEPS[@]}"; do
  if curl -sf -H "User-Agent: oxml-cli publish script (https://github.com/sebastienrousseau/oxml-cli)" "https://crates.io/api/v1/crates/$dep/$version" >/dev/null; then
...
for i in $(seq 1 30); do
  if curl -sf -H "User-Agent: oxml-cli publish script (https://github.com/sebastienrousseau/oxml-cli)" "https://crates.io/api/v1/crates/$CRATE/$version" >/dev/null; then
